...
class VkPhotos {
	...
	public function __construct() {
		...
		$this->arrayPictureSizeDESC = array(
			'5' => 'photo_2560',
			'4' => 'photo_1280',
			'3' => 'photo_807',
			'2' => 'photo_604',
			...
			<div class='wrap'>
				...
				if ( $dirstream = @opendir( $this->dirForCache ) ) {
					...
					while ( false !== ( $filename = readdir( $dirstream ) ) ) {
						...
						if ( ! is_dir( $this->dirForCache . $filename ) ) {
							continue;
						}
	...
	public function image_crop( $width = 200, $height = 200, $dir, $id, $file ) {
		...
		imagecopyresampled(
			...
			0,
			0,           // Координаты (x,y) верхнего левого угла области в новом изображении
			$x_e,
			$y_e,     // Координаты (x,y) верхнего левого угла области копируемого исходного изображения
